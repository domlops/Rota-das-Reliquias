...
function getPathLength(pathPoints) {
  ...
  for (let index = 1; index < pathPoints.length; index += 1) {
    totalLength += tspSystem.distanceBetween(pathPoints[index - 1], pathPoints[index]);
  }

  return totalLength;
}

function syncAfterDebugAction() {
  refreshBestRoute();
  updateStatusPanel();
  syncInventoryOverlay();
  render();
...
function findDebugTarget(targetId = "base") {
  const normalizedTargetId = String(targetId ?? "")
    .trim()
    .toLowerCase();

  if (normalizedTargetId === "base") {
    return state.base;
  }

  return (
    state.pickupPoints.find((pickupPoint) => pickupPoint.id.toLowerCase() === normalizedTargetId) ??
    state.pickupPoints.find((pickupPoint) => pickupPoint.label.toLowerCase() === normalizedTargetId) ??
    null
  );
}

function movePlayerToTarget(targetId) {
  const target = findDebugTarget(targetId);

  if (!target || !state.terrainMap) {
    return {
      moved: false,
  ...
  const pathPlan = tspSystem.calculatePath(getRouteStartPoint(), target, state.terrainMap);

  if (pathPlan.totalCost > state.stepsRemaining) {
    return {
      moved: false,
      reason: "insufficient_steps",
      requiredSteps: Number(pathPlan.totalCost.toFixed(2)),
      availableSteps: Number(state.stepsRemaining.toFixed(2)),
    };
  }

  state.player.x = target.x;
  state.player.y = target.y;
  state.distanceTraveled += getPathLength(pathPlan.pathPoints);
  state.stepsRemaining = Math.max(0, state.stepsRemaining - pathPlan.totalCost);
  syncAfterDebugAction();

  return {
    moved: true,
    targetId: target.id,
    stepsSpent: Number(pathPlan.totalCost.toFixed(2)),
  };
}
...
function getDebugSnapshot() {
  ...
  return {
    ...
    inventorySortCriterion: state.inventorySortCriterion,
    generationCount: state.generationCount,
    stepsRemaining: Number(state.stepsRemaining.toFixed(2)),
    distanceTraveled: Number(state.distanceTraveled.toFixed(2)),
    expeditionComplete: state.expeditionComplete,
    nearbyPickupId: state.nearbyPickupId,
    interactionMessage: state.interactionMessage,
    lastCollectedItemName: state.lastCollectedItemName,
    player: {
      x: Number(state.player.x.toFixed(2)),
      y: Number(state.player.y.toFixed(2)),
      directionLabel: state.player.directionLabel,
    },
    base: state.base
      ? {
          id: state.base.id,
    ...
    routePlan: state.routePlan
      ? {
          strategy: state.routePlan.strategy,
          totalDistance: Number(state.routePlan.totalDistance.toFixed(2)),
          orderedPointIds: state.routePlan.orderedPoints.map((pickupPoint) => pickupPoint.id),
          orderedPointLabels: state.routePlan.orderedPoints.map((pickupPoint) => pickupPoint.label),
        }
      : null,
    pickupPoints: state.pickupPoints.map((pickupPoint) => ({
      id: pickupPoint.id,
      label: pickupPoint.label,
      collected: pickupPoint.collected,
      x: pickupPoint.x,
      y: pickupPoint.y,
      remainingItems: getRemainingItems(pickupPoint).map((item) => item.nome),
    })),
    inventoryItems: getInventoryItems().map((item) => ({
      id: item.id,
      nome: item.nome,
...
window.__rotaDasReliquiasQa = {
  ...
  movePlayerToNextRoutePoint() {
    const nextPointId = state.routePlan?.orderedPoints?.[0]?.id;
    ...
    return movePlayerToTarget(nextPointId);
  },
  movePlayerToTarget,
  setInventorySortCriterion(criterion) {
    setInventorySortCriterion(criterion);
    syncAfterDebugAction();
  },
  startNewGame() {
